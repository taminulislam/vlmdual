"""
Train the DualStreamBaselineNet on the ACID_Journal extended_dataset_50x.

Entry point:
    python scripts/train_baseline.py --config configs/baseline.yaml

Features:
- AMP (fp16) training on A100/A40
- Warmup + cosine LR schedule
- Gradient clipping
- Early stopping on val_balanced_accuracy
- wandb offline logging
- Checkpoints: best_acc.pth, best_iou.pth, last.pth, epoch_N every 10
"""
...
# Make sibling modules importable no matter where we launch from.
_HERE = os.path.dirname(os.path.abspath(__file__))
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

from baseline_model import DualStreamBaselineNet            # noqa: E402
from dual_gas_dataset import build_dataloaders              # noqa: E402
from losses_metrics import CombinedLoss, compute_metrics, confusion_matrix_str  # noqa: E402
...
def validate(model, loader, loss_fn, device, cfg) -> Dict[str, Any]:
    ...
    all_cls_logits = []
    all_labels = []
    # We accumulate seg confusion counts incrementally to avoid blowing memory.
    ...
    # Classification metrics via the shared helper
    ...
    # Segmentation metrics from accumulators
    fg_iou = []
    fg_dice = []
    for c in range(1, seg_classes):
        ...
        if (pred_area[c] + tgt_area[c]).item() > 0:
            fg_dice.append(
                2 * inter_cls[c].item() / (pred_area[c].item() + tgt_area[c].item())
            )
...
def main():
    ...
    # --- Logging (wandb optional, stdout always) ---
    ...
    log(f"Model parameters: {n_params:.1f}M")
    ...
    for epoch in range(cfg["train"]["epochs"]):
        ...
        log(
            ...
            f"val_mean_dice={val['val_mean_dice']:.4f}  "
        ...
        if wandb_run is not None:
            ...
            for i in range(3):
                ...
                payload[f"val_prec_{CLASS_NAMES[i]}"] = val["val_per_class_precision"][i]
                payload[f"val_rec_{CLASS_NAMES[i]}"] = val["val_per_class_recall"][i]
